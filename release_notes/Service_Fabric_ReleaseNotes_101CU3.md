...
|-|-|-|
| [Service Fabric Runtime](https://download.microsoft.com/download/b/8/a/b8a2fb98-0ec1-41e5-be98-9d8b5abf7856/MicrosoftServiceFabric.10.1.2175.9590.exe) | Ubuntu 20 <br> Windows | 10.1.2108.1 <br> 10.1.2175.9590 |
| [Service Fabric for Windows Server](https://download.microsoft.com/download/8/3/6/836E3E99-A300-4714-8278-96BC3E8B5528/10.1.2175.9590/Microsoft.Azure.ServiceFabric.WindowsServer.10.1.2175.9590.zip) | Service Fabric Standalone Installer Package | 10.1.2175.9590 |
| [.NET SDK](https://download.microsoft.com/download/b/8/a/b8a2fb98-0ec1-41e5-be98-9d8b5abf7856/MicrosoftServiceFabricSDK.7.1.2175.msi) | Windows .NET SDK <br> Microsoft.ServiceFabric <br> Reliable Services and Reliable Actors <br> ASP.NET Core Service Fabric integration | 7.1.2175 <br> 10.1.2175 <br> 7.1.2175 <br> 7.1.2175 |
| [Java SDK](https://download.microsoft.com/download/b/8/a/b8a2fb98-0ec1-41e5-be98-9d8b5abf7856/MicrosoftServiceFabricSDK.7.1.2175.msi) | Java for Linux SDK | 1.0.6 |
| Service Fabric PowerShell and CLI | AzureRM PowerShell Module <br> SFCTL | 0.3.15 <br> 11.0.1 |

## Service Fabric Feature and Bug Fixes
| Versions | IssueType | Description | Resolution | 
|-|-|-|-|
| Windows -<br>10.1.2175.9590 <br> Ubuntu 20 -<br>10.1.2108.1 | Feature | .NET 8 Support | **Brief Description:** Support for .NET 8 in Service Fabric SDKs. <br> **Feature Impact:** Allow Service Fabric customers build and deploy services on .NET 8. <br> **Solution/Fix:** Microsoft.ServiceFabric.AspNetCore.* packages now include .NET 8 binaries. All Microsoft.ServiceFabric.* packages have been tested on .NET 8. <br> **Breaking Changes:** [.NET 8 has a number of changes](https://learn.microsoft.com/dotnet/core/compatibility/8.0) that may affect Service Fabric applications, including but not limited to:<br> - [BinaryFormatter is disabled by default and will be removed from .NET 9](https://github.com/dotnet/designs/blob/main/accepted/2020/better-obsoletion/binaryformatter-obsoletion.md). Service Fabric relies on the BinaryFormatter for passing exception information from remoting calls by default. Customers can still configure their services to use the unsafe binary serialization, however, in the next major version of Service Fabric SDK v8, binary serialization will be disabled by default. For a procedure to enable the BinaryFormatter, see the [BinaryFormatter Obsoletion Strategy GitHub page](https://github.com/dotnet/designs/blob/main/accepted/2020/better-obsoletion/binaryformatter-obsoletion.md). Customers should consider migrating their Service Fabric services to DataContract-based serialization as described in the [Remoting exception serialization overview](https://learn.microsoft.com/azure/service-fabric/service-fabric-reliable-services-exception-serialization).<br> - [Runtime-specific apps are no longer self-contained by default](https://learn.microsoft.com/dotnet/core/compatibility/sdk/8.0/runtimespecific-app-default). Customers need to explicitly set the `SelfContained` property to `true` in their projects or use a [service startup script](https://learn.microsoft.com/azure/service-fabric/service-fabric-run-script-at-service-startup) to [install the .NET 8 runtime](https://learn.microsoft.com/en-us/dotnet/core/tools/dotnet-install-script). |
| Windows -<br>10.1.2175.9590 <br> Ubuntu 20 -<br>10.1.2108.1 | Feature | Health | **Brief Description:** Remap hardware related JET error form StoreLvidLimitHit to a new "FABRIC_E_STORE_OUT_OF_LONG_VALUE_IDS" exception and reflect the change in the health event. <br> **Feature/Bug Impact:** Improve visibility of LVID exhaustion with a user-friendly exception and internally consistent naming. <br> **Solution/Fix:** Long Value IDs are used by ESE (internal storage engine used by KVS) to keep track of values greater than 5120 bytes. These IDs can be exhausted over a period and the database needs to be compacted to recover them. This exhaustion of IDs is currently an internal error code which cannot be expressed as a public exception or a status code. This work items replaces the internal error code with a new public error code FABRIC_E_STORE_OUT_OF_LONG_VALUE_IDS. |
| Windows -<br>10.1.2175.9590 <br> Ubuntu 20 -<br>10.1.2108.1 | Feature | MaxJetInstances | **Brief Description:** MaxJetInstances is marked as deprecated and removed from usage. <br> **Feature/Bug Impact:** As an unused setting with no reason to set outside of the default state, we removed this setting to prevent possible future issues. <br> **Solution/Fix:** This fixes any potential issues from a customer setting this variable to an unusable setting. |
| Windows -<br>10.1.2175.9590 <br> Ubuntu 20 -<br>10.1.2108.1 | Feature | Key Value Store (KVS) | **Brief Description:** The initial defragmentation of KVS replicas will now be distributed between an immediate defragmentation and the setting of MaxDefragFrequencyInMinutes which is by default 30 minutes. All subsequent defragmentations will use the MaxDefragFrequencyInMinutes value. Previously all initial  defragmentations were immediately scheduled upon open. <br> **Feature/Bug Impact:** A random integer is now generated in a range of (0, MaxDefragFrequencyInMinutes) to determine the initial delay for defragmentation instead of happening immediately to prevent scenarios where multiple replicas are open at the same time leading to increased disk usage. <br> **Solution/Fix:** This prevents scenarios where multiple replicas are open at the same time leading to increased disk usage even though customers have set a setting for a MaxDefragFrequencyInMinutes to once a day or even once a week. |
