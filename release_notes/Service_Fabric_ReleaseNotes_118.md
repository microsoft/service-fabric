...
| Bug fix | Reliable Collections metrics lifecycle and configuration improvements | **Impact:** Prevents metric worker thread leaks during replica lifecycle operations and allows metrics to be controlled per service. <br> **Solution/Fix:** Per-replica metric resources are now cleaned up reliably, preventing metric worker thread leaks during replica lifecycle operations. Reliable Collections metrics also honor the service-level `EnableMetrics` setting, so services receive the configured behavior. No action is required. |
| Feature | Native SDK support for partition endpoint version serialization | **Impact:** Clients can retain endpoint-version data across serialized partition results, improving stale-result detection. <br> **Solution/Fix:** A new Native SDK API allows partition endpoint version information to be serialized and deserialized. This preserves endpoint version data in service-resolution results and improves stale-result detection. No action is required. |
