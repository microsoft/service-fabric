...
|-|-|-|-|
| **Windows - 8.1.321.9590<br>Ubuntu 16 - 8.1.323.1<br>Ubuntu 18 - 8.1.323.1** | **Bug** | Upgrade of stateful and stateless services fail with Service DNS name in use error | **Brief desc**: Azure Srtvice Fabric Service Update fails with DnsNameInUse if ServiceDnsName is set. <br> **Impact**: Updating Service Fabric service via ARM is being blocked when ServiceDnsName is unchanged. This is blocking service update flows for some users. This is resulting from a new client parameterization that exposed an underlying behavior that blocked no-ops. <br> **Fix** NamingService is now handling the ServiceDnsName parameter as a no-op when it is unchanged. <br> **Workaround** Update service using Service Fabric PowerShell cmdlet Update-ServiceFabricService, omitting the ServiceDnsName parameter if it is unchanged. |
