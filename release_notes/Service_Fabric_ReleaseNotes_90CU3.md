...
|---------|----------|---------|
|Service Fabric Runtime| Ubuntu 18 <br> Ubuntu 20 <br> Windows | 9.0.1103.1 <br> 9.0.1093.1 <br> 9.0.1107.9590 |
...
## Key Announcements
* SFRP NodeTypes with VMSS Durability =”Silver”/”Gold” should always have atleast 5 VMs. If you need to deploy with less than 5 VMs, please consider using Durability = Bronze, but this is not recommended for Production clusters.This will be blocked in SFRP starting *09/30/2022*, and will start failing the deployments when the target VM count is less than 5 <br> For details see:[Durability characterstics of the cluster](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-cluster-capacity#durability-characteristics-of-the-cluster). Initially, this will be enforced on new clusters, but gradually this validation will be enabled for all clusters.
* SFRP NodeTypes with VMSS Durability =”Silver”/”Gold” should always have WindowsUpdate explicitly disabled to avoid unintended OS restarts due to the Windows updates, which can impact the availability of the workloads or the cluster itself. This can be achieved by setting the "enableAutomaticUpdates": false, in the VMSS OSProfile.Instead consider enabling Automatic VMSS Image upgrades.<br>
...
This will be blocked in SFRP starting *09/30/2022*, and will start failing the deployments when the WindowsUpdates are not disabled on the VMSS. Initially, this will only be enforced on new clusters, but gradually we will enable this validation for all clusters.
...
|| Windows Developer Set-up| 9.0.1107.9590 | N/A | https://download.microsoft.com/download/b/8/a/b8a2fb98-0ec1-41e5-be98-9d8b5abf7856/MicrosoftServiceFabric.9.0.1048.9590.exe |
|Service Fabric for Windows Server |Service Fabric Standalone Installer Package | 9.0.1107.9590 |N/A | https://download.microsoft.com/download/8/3/6/836E3E99-A300-4714-8278-96BC3E8B5528/9.0.1048.9590/Microsoft.Azure.ServiceFabric.WindowsServer.9.0.1048.9590.zip |
||Service Fabric Standalone Runtime | 9.0.1107.9590 |N/A | https://download.microsoft.com/download/3/1/F/31F3FEEB-F073-4E27-A98B-8E691FF74F40/ServiceFabric.9.0.1107.9590.cab |
|.NET SDK |Windows .NET SDK | 6.0.1107 |N/A | https://download.microsoft.com/download/b/8/a/b8a2fb98-0ec1-41e5-be98-9d8b5abf7856/MicrosoftServiceFabricSDK.6.0.1048.msi  |
