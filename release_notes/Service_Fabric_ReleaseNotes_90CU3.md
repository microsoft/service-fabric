...
## Key Announcements
* SFRP node types with VMSS Durability of ”Silver”/”Gold” should always have at least 5 VMs. If you need to deploy with less than 5 VMs, please consider using Durability "Bronze", but this is not recommended for Production clusters. Node types with VMSS Durability of ”Silver”/”Gold” will be blocked in SFRP starting *09/30/2022*, and will start failing the deployments when the target VM count is less than 5. Initially, this will be enforced on new clusters, and will soon be enabled for all clusters. <br> For details see: [Durability characteristics of the cluster] (https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-cluster-capacity#durability-characteristics-of-the-cluster). 
* SFRP node types with VMSS Durability of ”Silver”/”Gold” should always have Windows update explicitly disabled to avoid unintended OS restarts due to the Windows updates, which can impact the availability of the workloads or the cluster itself. This can be achieved by setting the "enableAutomaticUpdates": false, in the VMSS OSProfile. Consider enabling Automatic VMSS Image upgrades instead. The deployments will start failing from *09/30/2022* for new clusters, if the WindowsUpdates are not disabled on the VMSS. This validation will be enabled gradually for all clusters.<br>
