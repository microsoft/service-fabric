...
## Key Announcements
* SFRP NodeTypes with VMSS Durability =”Silver”/”Gold” should always have atleast 5 VMs. If you need to deploy with less than 5 VMs, please consider using Durability = Bronze, but this is not recommended for Production clusters. For details see:[Durability characterstics of the cluster](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-cluster-capacity#durability-characteristics-of-the-cluster).
* SFRP NodeTypes with VMSS Durability =”Silver”/”Gold” should always have WindowsUpdate explicitly disabled, to avoid unintended OS restarts due to the windows updates, which can impact the availability of the workloads or the cluster itself. This can be achieved by setting the "enableAutomaticUpdates": false, in the VMSS OSProfile:

    "osProfile": {
        "windowsConfiguration": {
            "enableAutomaticUpdates": false
Instead consider enabling Automatic VMSS Image upgrades: [VMSS Image Upgrades](https://docs.microsoft.com/en-us/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-automatic-upgrade) documentation
This will be blocked in SFRP starting *09/30/2022*, and will start failing the deployments when the target VM count is less than 5 or the WindowsUpdates are not disabled on the VMSS. Initially, this will only be enforced on new clusters, but gradually we will enable this validation for all clusters.
