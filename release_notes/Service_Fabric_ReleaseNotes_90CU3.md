...
This will be blocked in SFRP starting *09/30/2022*, and will start failing the deployments when the WindowsUpdates are not disabled on the VMSS. Initially, this will only be enforced on new clusters, but gradually this validation will be enabled for all clusters.
