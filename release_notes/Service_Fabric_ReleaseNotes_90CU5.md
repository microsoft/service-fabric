...
|-|-|-|-|
| **Windows - 9.1.1436.9590<br>Ubuntu 18 - 9.1.1230.1<br>Ubuntu 20 - 9.1.1230.1** | **Bug** | Backup Restore Service(BRS) | **Brief Description**: Customers using plain text credentials in backup policy will have a persistent health warning on BRS to notify users to make use of token-based backup storage type.<br>**Fix**: Azure Storage Connection String backup storage type is in process of deprecation. From SF 10.0 release(Apr 2023), only token based backup storage types will be supported.
