...
| **Windows - 9.1.1390.9590<br>Ubuntu 18 - 9.1.1206.1<br>Ubuntu 20 - 9.1.1206.1** | **Bug** | Managed Keyvault References | **Brief Description**: When deploying an application that uses Managed Key Vault References, customers may previously notice that despite the application starting successfully, the logical “application” resource deployment may fail with AppUpgradeConfigDifferentThanTarget.<br>**Fix**: The code bug was fixed in the backend logic to evaluate app deployment success.<br>Documentation Reference: Read more about Managed Key Vault references at Azure Service Fabric here: [Service Fabric application KeyVault references](https://learn.microsoft.com/en-us/azure/service-fabric/service-fabric-keyvault-references)
| **Windows - 9.1.1390.9590<br>Ubuntu 18 - 9.1.1206.1<br>Ubuntu 20 - 9.1.1206.1** | **Bug** | Managed Keyvault References | **Brief Description**: When using Managed Key Vault References, user-triggered upgrades will take precedence over secret-rotation-triggered upgrades. Previously if a user-triggered application deployment occurred close to a secret rotation, the system-triggered upgrade might cause the user-triggered upgrade to rollback. Now, the system will wait for user-triggered upgrades to complete.<br>**Fix**:  Improvmements were made in Central Secret Service’s secret rotation engine to prevent unexpected rollbacks<br>Documentation Reference: Read more about Managed Key Vault references at Azure Service Fabric here: [Service Fabric application KeyVault references](https://learn.microsoft.com/en-us/azure/service-fabric/service-fabric-keyvault-references)
