...
|-|-|-|-|
| **Windows - 9.0.1107.9590<br>Ubuntu 18 - 9.0.1103.1<br>Ubuntu 20 - 9.0.1103.1** | **Bug** | Cluster Manager service | **Brief Description**: If Upgrade application was called against an application that did not exist then "ApplicationTypeNotFound" is returned instead of "ApplicationNotFound".<br>**Fix**: The code bug in Cluster Manager is fixed to return the correct error message of "ApplicationNotFound"
| **Windows - 9.0.1107.9590<br>Ubuntu 18 - 9.0.1103.1<br>Ubuntu 20 - 9.0.1103.1** | **Bug** | Managed Keyvault References | **Brief Description**: When deploying an application that uses Managed Key Vault References, end users may notice application deployment failing despite the application starting successfully.<br>**Fix**: This change ensures application that uses Managed Key Vault References deployments are successful.<br>Documentation Reference: Read more about Managed Key Vault references at Azure Service Fabric here: [Using Service Fabric application KeyVault references - Azure Service Fabric | Microsoft Learn](https://learn.microsoft.com/en-us/azure/service-fabric/service-fabric-keyvault-references)
| **Windows - 9.0.1107.9590<br>Ubuntu 18 - 9.0.1103.1<br>Ubuntu 20 - 9.0.1103.1** | **Bug** | Managed Keyvault References | **Brief Description**: When using Managed Key Vault References, user-triggered upgrades will take precedence over secret-rotation-triggered upgrades causing unexpected rollbacks during application upgrades<br>**Fix**:  Improvmements were made in Central Secret Service’s secret rotation engine to prevent unexpected rollbacks<br>Documentation Reference: Read more about Managed Key Vault references at Azure Service Fabric here: [Using Service Fabric application KeyVault references - Azure Service Fabric | Microsoft Learn](https://learn.microsoft.com/en-us/azure/service-fabric/service-fabric-keyvault-references)
| **Windows - 9.0.1107.9590<br>Ubuntu 18 - 9.0.1103.1<br>Ubuntu 20 - 9.0.1103.1** | **Feature** | Service Fabric Explorer | **Brief Description**: Make infrastructure service issues evident to the customers in Service Fabric explorer by adding summary tab to show concerning repair jobs and throttled repair jobs to improve debuggability for end users.<br>For more information see: [SFX Improvements](https://github.com/microsoft/service-fabric-explorer/issues/776)
...
* Ubuntu 18.04 LTS will reach its 5-year end-of-life window on April 30, 2023. Service Fabric runtime will drop support for 18.04 LTS, and we recommend moving your clusters and applications to Ubuntu 20.04 LTS. Current applications running on it will continue to work, but requests to investigate issues w.r.t this OS or requests for change will no longer be entertained. Service Fabric runtime will stop producing builds for U18.04 so customers will no longer be able to get SF related patches.
