...
| **Windows - 9.1.1390.9590<br>Ubuntu 18 - 9.1.1206.1<br>Ubuntu 20 - 9.1.1206.1** | **Bug** | Cluster Resource Manager | **Brief Description**: Dynamic Node capacities and Node properties feature was removed due to below problems:<ol><li> Update to Failover Manager Master(FMM) was not sent when dynamic Node properties were updated. This led to bad placement decisions of Failover Manager(FM) service.</li><li>A Node in starting up state in a cluster(NodeUp event) can be rejected. If the rejected node manages to join the cluster, Node properties are not read correctly from the manifest, which can impact placement of all services in the cluster.</li></ol> **Fix**: Support for dynamic Node capacities and Node properties was removed to resolve the issue.
