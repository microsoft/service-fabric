...
* SF Runtime will support .Net 7 from 9.1CU2. 
...
| **Windows - 9.1.1436.9590<br>Ubuntu 18 - 9.1.1230.1<br>Ubuntu 20 - 9.1.1230.1** | **Bug** | Backup Restore Service(BRS) | **Brief Description**: If SF cluster is upgraded to 8.2.1686.9590 / 9.0.1107.9590 / 9.1.1387.9590 which has existing backup policies, post upgrade BRS fails to deserialize old metadata. It will stop taking backup and restore on the partition/service/app in question, though cluster and BRS remains healthy <br>**Fix**: The issue with backup and restore functionality is fixed. 
...
| **Windows - 9.1.1436.9590<br>Ubuntu 18 - 9.1.1230.1<br>Ubuntu 20 - 9.1.1230.1** | **Bug** | Cluster Resource Manager | **Brief Description**: Dynamic Node capacities and Node properties feature was removed due to below problems:<ol><li> Update to Failover Manager Master(FMM) was not sent when dynamic Node properties were updated. This led to bad placement decisions of Failover Manager(FM) service.</li><li>A Node in starting up state in a cluster(NodeUp event) can be rejected. If the rejected node manages to join the cluster, Node properties are not read correctly from the manifest, which can impact placement of all services in the cluster.</li></ol> **Fix**: Support for dynamic Node capacities and Node properties was removed to resolve the issue.
