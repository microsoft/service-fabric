...
extern "C" int wmemcmp(const wchar_t * src1, const wchar_t * src2, size_t n)
{
	int diff = 0;
...
wmemchr(const wchar_t * src, wchar_t val, size_t n)
{
	while (n)
	{
		if (*src == val) return const_cast<wchar_t*>(src);
		src++;
		n--;
	}
...
{
	return static_cast<wchar_t*>( memmove(dst, src, n * sizeof(wchar_t)) );
}
...
{
	return static_cast<wchar_t*>( memcpy(dst, src, n * sizeof(wchar_t)) );
}
