...
This file includes WMI support for Service Fabric BlockStore Device's interaction with the BlockStore Service.
...
/*                                                                                                */                     
/**************************************************************************************************/                     
#define IdxSFBDMPDriver_GUID                         0             // Index value of SFBDMPDriver_GUID in GuidList[].
#define IdxSFBDMPDriver2_GUID                        1             // Index value of SFBDMPDriver2_GUID in GuidList[].
#define IdxSFBDMPDriverSrbActivity_GUID              2             // Index value of SFBDMPDriverSrbActivity_GUID in GuidList[].
#define IdxGmDrvDrvMethodGuid                        3             // Index value of SFBDf2DrvMethod_GUID in GuidList[].
...
SCSIWMIGUIDREGINFO WmiGuidList[] =                    // GUIDs supported.                       
  {          
   {&SFBDMPDriver_GUID,                 1, WMIREG_FLAG_INSTANCE_PDO},
   {&SFBDMPDriver2_GUID,                1, WMIREG_FLAG_INSTANCE_PDO},
   {&SFBDMPDriverSrbActivity_GUID,      1, WMIREG_FLAG_INSTANCE_PDO},
...
/**************************************************************************************************/                     
VOID
InitializeWmiContext(IN pHW_HBA_EXT pHbaExtension)
{
                 ...
                 )
{
    ...
    switch(GuidIndex) {
        case IdxSFBDMPDriver_GUID:

            // Demo index.

            sizeNeeded = SFBDMPDriver_SIZE;
            ...
            {
                  PSFBDMPDriver pOut =                // Point to output area at next 8-byte offset.
                    (PSFBDMPDriver)((PUCHAR)pBuffer + ulBfrUsed);
        ...
        case IdxSFBDMPDriverSrbActivity_GUID:
            ...
            switch(MethodId) {
                ...
                case GetFcpTargetMapping: {
                    ...
                    sizeNeeded = FIELD_OFFSET(GetFcpTargetMapping_OUT, Entry) +               // Fixed portion.
                                 (pHbaExtension->LUNPerHBA * sizeof(HBAFCPScsiEntry));    // Variable portion.
                    ...
                    if (OutBufferSize >= sizeNeeded) {
                        ...
                        pOut->TotalEntryCount = pHbaExtension->LUNPerHBA;

                        pOut->OutEntryCount = pHbaExtension->LUNPerHBA;

                        for (i=0; i<pHbaExtension->LUNPerHBA; i++) {  // Construct each HBAFCPScsiEntry.
        ...
        case IdxMSFC_HBAAdapterMethods_GUID:
            ...
            switch(MethodId) {
                ...
                case ScsiInquiry: {
                    ...
                    pInqData->DeviceType     = DIRECT_ACCESS_DEVICE;
                    ...
                    RtlMoveMemory(pInqData->ProductId, PRODUCT_ID_ascii, sizeof(PRODUCT_ID_ascii));
                    RtlMoveMemory(pInqData->ProductRevisionLevel, PRODUCT_REV_ascii, sizeof(PRODUCT_REV_ascii));
                    ...
                    if (InBufferSize >= sizeof(ULONG)) {
                        ...
                        if (InBufferSize >= sizeNeeded) {
                            ...
                            if (OutBufferSize >= sizeNeeded) {
                                ...
                                pReadCapData->BytesPerBlock = SECTOR_SIZE;
                            } else {
                                status = SRB_STATUS_DATA_OVERRUN;
                            }
                        } else {
