...
namespace Threadpool{
    ...
    int HillClimbing::Update(int currentThreadCount, double sampleDuration, int numCompletions, int* pNewSampleInterval)
    {
        ...
        if (sampleCount > m_wavePeriod)
        {
            ...
            if (averageThroughput > 0 && averageThreadCount > 0)
            {
                ...
                if (adjacentPeriod2 <= sampleCount)
                    throughputErrorEstimate = max(throughputErrorEstimate,
        ...
        m_currentControlSetting = min(m_threadpoolMgr->MaxLimitTotalWorkerThreads-newThreadWaveMagnitude, m_currentControlSetting);
        m_currentControlSetting = max(m_threadpoolMgr->MinLimitTotalWorkerThreads, m_currentControlSetting);
        ...
        newThreadCount = min(m_threadpoolMgr->MaxLimitTotalWorkerThreads, newThreadCount);
        newThreadCount = max(m_threadpoolMgr->MinLimitTotalWorkerThreads, newThreadCount);
