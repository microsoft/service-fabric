...
#pragma once
//#define THREADPOOL_ENABLE_LIFO_SEMAPHORE  1
...
namespace Threadpool {
    ...
    {
    public:
        explicit LifoEvent() : signalled(false), timedout(false)
        {
        ...
        inline bool WaitOne(unsigned int milliseconds)
        {
            timespec ts;
            clock_gettime(CLOCK_REALTIME, &ts);
            ts.tv_sec += milliseconds / 1000;
    ...
    public:
        ...
        inline bool Wait(unsigned int milliseconds)
        {
            ...
            queue_.emplace_front(myEvent);
            auto iter = queue_.begin();
            pthread_mutex_unlock(&mutex_);

            bool signaled = myEvent->WaitOne(milliseconds);
        ...
        inline bool Release(uint count)
        {
            ...
            value_ += count;
            auto iter = queue_.begin();
            while (iter != queue_.end() && value_)
            {
                if (!(*iter++)->Set()) continue;
                --value_;
            }
            pthread_mutex_unlock(&mutex_);
            return true;
    ...
    private:
        list<shared_ptr<LifoEvent>> queue_;
        pthread_mutex_t mutex_;
        int value_;
    };
