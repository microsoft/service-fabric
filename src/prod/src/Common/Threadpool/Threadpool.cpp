...
#define THREADPOOL_PERF_TRACE(msg, cpuutil)                                                   \
    ...
    BOOL ThreadpoolMgr::Initialize()
    {
        ...
        RetiredWorkerSemaphore = new LifoSemaphore();
#else
    ...
    DWORD ThreadpoolMgr::GetDefaultMaxLimitWorkerThreads(DWORD minLimit)
    {
        ...
        limit = min(limit, (ULONGLONG)ThreadCounter::MaxPossibleCount);
        return (DWORD)limit;
    }
    ...
    void ThreadpoolMgr::AdjustMaxWorkersActive()
    {
        ...
        if (elapsed * 1000.0 >= (ThreadAdjustmentInterval / 2))
        {
            ...
            ThreadAdjustmentInterval = max(newInterval, HillClimbingConfig::SampleIntervalLow);
            ThreadAdjustmentInterval = min(ThreadAdjustmentInterval, HillClimbingConfig::SampleIntervalHigh);
    ...
    RetryWaitForWork:
        ...
        if (!pThis->WorkerSemaphore->Wait(ThreadpoolConfig::WorkerUnfairSemaphoreTimeout))
        {
            ...
            while (true)
            {
                ...
                newCounts.MaxWorking = max(pThis->MinLimitTotalWorkerThreads, min(newCounts.NumActive, newCounts.MaxWorking));
                oldCounts = pThis->WorkerCounter.CompareExchangeCounts(newCounts, counts);
                if (oldCounts == counts)
                {
    ...
    void* ThreadpoolMgr::GateThreadStart(LPVOID lpArgs)
    {
        ...
        do
        {
            ...
            int throttle = GetThreadpoolThrottle();
            if (throttle != pThis->MaxLimitTotalWorkerThreads)
            {
                pThis->MaxLimitTotalWorkerThreads = throttle;
                TP_TRACE(Info, "Setting MaxLimitTotalWorkerThreads to %d", throttle);
            }
            ...
            if(epoch++ % (ThreadpoolConfig::GaterThreadPeriodicDumpTime / ThreadpoolConfig::GateThreadDelay) == 0)
            {
                ...
                TP_TRACE(Info, "Periodic Dump of Threadpool Internal: "
                        "Total-finished %d, "
                        "QSize %d, QAge %d ms, Working/Active/Retired/Max %d/%d/%d/%d, "
                        "HC-Adj-Interval %d, CPU-Util %d, ",
                         VolatileLoad(&pThis->TotalCompletedWorkRequests),
            ...
            if (0 == ThreadpoolConfig::DisableStarvationDetection)
            {
                        ...
                        pThis->SufficientDelaySinceLastDequeue())
                {
                    ...
                    {
                        ThreadCounter::Counts newCounts = counts;
                        newCounts.MaxWorking = min(counts.NumActive + forceIncrease, pThis->MaxLimitTotalWorkerThreads);

                        ThreadCounter::Counts oldCounts = pThis->WorkerCounter.CompareExchangeCounts(newCounts, counts);
                        if (oldCounts == counts)
                        {
                            THREADPOOL_PERF_TRACE("Gater StarvationDetection and ForceChange %lu. CPU Util: %d", pThis->CpuUtilization)
                            {
                                DangerousSpinLock::Holder tal(&pThis->ThreadAdjustmentLock);
                    ...
                    THREADPOOL_PERF_TRACE("Gater StarvationDetection and MaybeAddWorkingWorker %lu. CPU Util: %d", pThis->CpuUtilization)
                    {
                        pThis->MaybeAddWorkingWorker();
                    }
                }
