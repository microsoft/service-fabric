...
#pragma region test functions
    public:
        ...
        ktl::Awaitable<void> UndoFalseProgress_UndoAdd_ShouldSucceed_Test()
        {
            ...
            for (ULONG32 idx = 0; idx < count; idx++)
            {
                co_await VerifyKeyDoesNotExistInStoresAsync(ToBuffer(idx));
            }
            co_return;
        }
        ...
        ktl::Awaitable<void> UndoFalseProgress_UndoUpdate_ShouldSucceed_Test()
        {
...

            {
                ...
                for (ULONG32 idx = 0; idx < count; idx++)
                {
                    bool success = co_await Store->ConditionalUpdateAsync(*tx1->StoreTransactionSPtr, ToBuffer(idx), ToBuffer(idx + 1), DefaultTimeout, CancellationToken::None);
            ...
            for (ULONG32 idx = 0; idx < count; idx++)
            {
                co_await VerifyKeyExistsInStoresAsync(ToBuffer(idx), MakeBuffer(0), ToBuffer(idx), EqualityFunction);
            }
            co_return;
        }
        ...
        ktl::Awaitable<void> UndoFalseProgress_UndoRemove_ShouldSucceed_Test()
        {
...

            {
                ...
                for (ULONG32 idx = 0; idx < count; idx++)
                {
                    bool success = co_await Store->ConditionalRemoveAsync(*tx1->StoreTransactionSPtr, ToBuffer(idx), DefaultTimeout, CancellationToken::None);
                    CODING_ERROR_ASSERT(success);
                }
        ...
        ktl::Awaitable<void> NotifyAll_CRUD_ShouldSucceed_Test()
        {
...

            {
                ...
                co_await Store->AddAsync(*txn->StoreTransactionSPtr, ToBuffer(1), MakeBuffer(bufferSize, 1), DefaultTimeout, CancellationToken::None);
                co_await Store->AddAsync(*txn->StoreTransactionSPtr, ToBuffer(2), MakeBuffer(bufferSize, 2), DefaultTimeout, CancellationToken::None);
                co_await Store->AddAsync(*txn->StoreTransactionSPtr, ToBuffer(3), MakeBuffer(bufferSize, 3), DefaultTimeout, CancellationToken::None);
            ...
            {
                auto txn = CreateWriteTransaction();
                co_await Store->ConditionalUpdateAsync(*txn->StoreTransactionSPtr, ToBuffer(2), MakeBuffer(bufferSize, 22), DefaultTimeout, CancellationToken::None);
                co_await txn->CommitAsync();
            ...
            {
                auto txn = CreateWriteTransaction();
                co_await Store->ConditionalRemoveAsync(*txn->StoreTransactionSPtr, ToBuffer(3), DefaultTimeout, CancellationToken::None);
                co_await txn->CommitAsync();
        ...
        ktl::Awaitable<void> VerifyCount_AddUpdateRemoveMany_ShouldSucceed_Test()
       {
           ...
           for (int i = 1; i <= numItems; i++)
           {
               ...
               {
                   auto txn = CreateWriteTransaction();
                   co_await Store->AddAsync(*txn->StoreTransactionSPtr, ToBuffer(key), ToBuffer(value), DefaultTimeout, CancellationToken::None);
                   co_await txn->CommitAsync();
               ...
               {
                   auto txn = CreateWriteTransaction();
                   co_await Store->ConditionalUpdateAsync(*txn->StoreTransactionSPtr, ToBuffer(key), ToBuffer(value), DefaultTimeout, CancellationToken::None);
                   co_await txn->CommitAsync();
                   VerifyCountInAllStores(i);
               }
               ...
               {
                   auto txn = CreateWriteTransaction();
                   co_await Store->ConditionalRemoveAsync(*txn->StoreTransactionSPtr, ToBuffer(key), DefaultTimeout, CancellationToken::None);
                   co_await txn->CommitAsync();
        ...
        ktl::Awaitable<void> VerifyCount_CorrectAfterRecovery_ShouldSucceed_Test()
       {
           ...
           for (int i = 1; i <= numItems; i++)
           {
               ...
               if (i % 3 == 1)
               {
                   ...
                   {
                       auto txn = CreateWriteTransaction();
                       co_await Store->ConditionalUpdateAsync(*txn->StoreTransactionSPtr, ToBuffer(key), ToBuffer(value+1), DefaultTimeout, CancellationToken::None);
                       co_await txn->CommitAsync();
                   }
               }
        ...
        ktl::Awaitable<void> VerifyCount_AfterFalseProgressUndo_ShouldSucceed_Test()
       {
        ...
        
           {
               ...
               for (int i = 8; i < 24; i++)
               {
                   co_await Store->ConditionalUpdateAsync(*txn->StoreTransactionSPtr, ToBuffer(i), ToBuffer(i + 1), DefaultTimeout, CancellationToken::None);
               }

               for (int i = 16; i < 40; i++)
               {
                   co_await Store->ConditionalRemoveAsync(*txn->StoreTransactionSPtr, ToBuffer(i), DefaultTimeout, CancellationToken::None);
               }

               for (int i = 32; i < 48; i++)
               {
                   co_await Store->AddAsync(*txn->StoreTransactionSPtr, ToBuffer(i), ToBuffer(i+2), DefaultTimeout, CancellationToken::None);
               }
