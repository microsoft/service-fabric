   ...
   for (ULONG32 index = 0; index < lockHashTables_.Count(); index++)
   {
      ...
      while (enumerator->MoveNext())
      {
         ...
         for (ULONG32 lIndex = 0; lIndex < lockHashValueSPtr->ResourceControlBlock->WaitingQueue->Count(); lIndex++)
         {
            ...
            //
            // todo: Complete the pending waiter task.
            //
            //var captureX = x;
     ...
     __in Common::TimeSpan timeout/*, CancellationToekn*/)
{
 ...
 UnlockStatus::Enum LockManager::ReleaseLock(__in LockControlBlock& acquiredLock)
 {
    ...
    if (lockHashTableSPtr->LockEntries->TryGetValue(acquiredLock.LockResourceNameHash, lockHashValueSPtr))
    {
       ...
       ULONG32 clearLocksCount = 0;
       if (lockReleasedCleanupInProgress_[lockHashTableIndex])
       {
          KSharedPtr<DictionaryEnumerator<ULONG64, LockHashValue::SPtr>> enumerator = nullptr;
          NTSTATUS status = DictionaryEnumerator<ULONG64, LockHashValue::SPtr>::Create(*lockHashTableSPtr->LockEntries, GetThisAllocator(), enumerator);
          Diagnostics::Validate(status);
          ...
          {
             KeyValuePair<ULONG64, LockHashValue::SPtr> entry = enumerator->Current();
             LockHashValue::SPtr lLockHashValueSPtr = entry.Value;
             if (lLockHashValueSPtr->ResourceControlBlock->GrantedList->Count() == 0 && lLockHashValueSPtr->ResourceControlBlock->WaitingQueue->Count() == 0)
             {
                clearLocksCount++;
             }

             if (clearLocksCount > clearLocksThreshold_)
             {
                break;
             }
          }
       ...
       if (clearLocksCount > clearLocksThreshold_)
       {
          ...
          // todo: clear locks in the background
       ...
       if (!StoreUtilities::ContainsKey<LockControlBlock>(*(lockHashValueSPtr->ResourceControlBlock->GrantedList), acquiredLock))
       {
          //
          // Release second level lock.
          //
       ...
       if (acquiredLock.LockCount == 0)
       {
          ...
          //
          auto index = StoreUtilities::GetIndex<LockControlBlock>(*lockHashValueSPtr->ResourceControlBlock->GrantedList, acquiredLock);
          KInvariant(index >= 0);
    ...
    __in bool isExpired)
 {
    ...
    for (ULONG32 index = 0; index < waitersWokenUpSuccess.Count(); index++)
    {
       ...
       auto arrayIndex = StoreUtilities::GetIndex<LockControlBlock>(*lockHashValueSPtr->ResourceControlBlock->WaitingQueue, *completedWaiter);
    ...
    //
    if (lockHashValueSPtr->ResourceControlBlock->GrantedList->Count() == 0 &&
       lockHashValueSPtr->ResourceControlBlock->WaitingQueue->Count() == 0)
    {
       if (!status_)
       {
          // todo
 ...
 bool LockManager::ExpireLock(__in LockControlBlock& lockControlBlock)
 {
    ...
    if (lockHashTableSPtr->LockEntries->TryGetValue(lockControlBlockSPtr->LockResourceNameHash, lockHashValueSPtr))
    {
       ...
       //
       auto idx = StoreUtilities::GetIndex<LockControlBlock>(*lockHashValueSPtr->ResourceControlBlock->WaitingQueue, *lockControlBlockSPtr);
       if (idx == -1)
       {
          //
          // Release second level lock.
