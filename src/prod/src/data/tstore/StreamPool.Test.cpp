...
namespace TStoreTests
{
   ...
   public:
      ...
      ktl::Awaitable<ktl::io::KFileStream::SPtr> CreateFileStream()
      {
         ...
         NTSTATUS status = SyncAwait(FileFormatTestHelper::CreateBlockFile(fileName, GetAllocator(), CancellationToken::None, fileSPtr, streamSPtr_));
      ...
      BOOST_AUTO_TEST_CASE(NewPool_ShouldBeEmpty)
   {
      ...
      SyncAwait(streamPool->CloseAsync());
   }
   ...
   BOOST_AUTO_TEST_CASE(AcquireStream_OneCaller_ShouldCreateOneNewStream)
   {
      ...
      auto stream1 = SyncAwait(streamPool->AcquireStreamAsync());
      ...
      SyncAwait(streamPool->ReleaseStreamAsync(*stream1));
      ...
      auto stream2 = SyncAwait(streamPool->AcquireStreamAsync());
      ...
      SyncAwait(streamPool->ReleaseStreamAsync(*stream2));
   ...
   BOOST_AUTO_TEST_CASE(AcquireStream_TwoCallers_ShouldCreateTwoNewStreams)
   {
      ...
      auto stream3 = SyncAwait(streamPool->AcquireStreamAsync());
      auto stream4 = SyncAwait(streamPool->AcquireStreamAsync());
      ...
      SyncAwait(streamPool->ReleaseStreamAsync(*stream3));
      SyncAwait(streamPool->ReleaseStreamAsync(*stream4));
   ...
   BOOST_AUTO_TEST_CASE(ReleaseStream_MoreStreamsThanDesiredMax_ShouldDropStreams)
   {
      ...
      auto stream3 = SyncAwait(streamPool->AcquireStreamAsync());
      auto stream4 = SyncAwait(streamPool->AcquireStreamAsync());
      auto stream5 = SyncAwait(streamPool->AcquireStreamAsync());
      auto stream6 = SyncAwait(streamPool->AcquireStreamAsync());
      ...
      SyncAwait(streamPool->ReleaseStreamAsync(*stream5));
      ASSERT_IFNOT(streamPool->Count() == 3, "StreamPool.ReleaseStream() should have dropped the 5th stream.");
      SyncAwait(streamPool->ReleaseStreamAsync(*stream6));
