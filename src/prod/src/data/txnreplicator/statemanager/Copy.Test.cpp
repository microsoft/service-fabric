...
{
    class CopyTest : public StateManagerTestBase
    {
    ...
    public:
        ...
        Awaitable<void> Test_Apply_FalseProgress_Reverted(
            __in ULONG32 numberOfItems) noexcept;
    ...
    private:
        Awaitable<void> CopyTest::CopyTask(
    ...
    Awaitable<void> CopyTest::Test_Copy_MultipleStateProvider_AllActiveCopied(
        __in ULONG32 numberOfAdds, 
        __in ULONG32 numberOfRemoves,
        __in bool checkpointAfterAdd, 
        __in bool recoverInPlace) noexcept
    ...
    Awaitable<void> CopyTest::Test_SecondaryApply_Duplicate_AlreadyDeleted_NoopAndNoContext(
        __in ULONG32 numberOfAdds,
        __in ULONG32 numberOfRemoves,
        __in bool recoverInPlace) noexcept
    {
    ...
    Awaitable<void> CopyTest::Test_SecondaryApply_New_StateProvidersAdded(
    ...
    Awaitable<void> CopyTest::Test_Apply_CopyEmpty_InsertAndDelete_ItemDeleted(
        __in ULONG32 numberOfItems, 
        __in bool recoverInPlace) noexcept
    {
        NTSTATUS status;
    ...
    Awaitable<void> CopyTest::Test_Apply_CopyInsert_Delete_ItemDeleted(
        __in ULONG32 numberOfItems,
        __in bool recoverInPlace) noexcept
    {
        NTSTATUS status;
    ...
    Awaitable<void> CopyTest::Test_Apply_CopyDeleted_InsertAndDelete_ItemDeleted(
        __in ULONG32 numberOfItems, 
        __in bool replayInsert,
        __in bool recoverInPlace) noexcept
    {
    ...
    Awaitable<void> CopyTest::Test_Apply_FalseProgress_Reverted(
        __in ULONG32 numberOfItems) noexcept
    {
        NTSTATUS status = STATUS_UNSUCCESSFUL;

        // Expected
        KArray<KUri::CSPtr> checkpointList(GetAllocator(), numberOfItems);
        for (ULONG index = 0; index < numberOfItems; index++)
        {
            auto name = GetStateProviderName(NameType::Random);
            status = checkpointList.Append(name);
            CODING_ERROR_ASSERT(NT_SUCCESS(status));
        }
        
        KArray<KUri::CSPtr> insertList(GetAllocator(), numberOfItems);
        for (ULONG index = 0; index < numberOfItems; index++)
        {
            auto name = GetStateProviderName(NameType::Random);
            status = insertList.Append(name);
            CODING_ERROR_ASSERT(NT_SUCCESS(status));
        }

        KArray<KUri::CSPtr> deleteList(GetAllocator(), numberOfItems);
        for (ULONG index = 0; index < numberOfItems; index++)
        {
            auto name = GetStateProviderName(NameType::Random);
            status = deleteList.Append(name);
            CODING_ERROR_ASSERT(NT_SUCCESS(status));
        }

        // Setup
        KArray<LONG64> insertIdList(GetAllocator(), numberOfItems);
        KArray<LONG64> deleteIdList(GetAllocator(), numberOfItems);
        LONG64 checkpointLSN = FABRIC_INVALID_SEQUENCE_NUMBER;
        {
            co_await testTransactionalReplicatorSPtr_->OpenAsync(CancellationToken::None);
            co_await testTransactionalReplicatorSPtr_->ChangeRoleAsync(FABRIC_REPLICA_ROLE_PRIMARY, CancellationToken::None);

            // Add checkpointList and Checkpoint
            co_await PopulateAsync(checkpointList);

            // Add deleteList that we will add, remove and false progress the remove.
            co_await PopulateAsync(deleteList);
            deleteIdList = PopulateStateProviderIdArray(deleteList);

            checkpointLSN = co_await testTransactionalReplicatorSPtr_->CheckpointAsync(CancellationToken::None);

            // Add insertList that we will add an false progress.
            co_await PopulateAsync(insertList);
            insertIdList = PopulateStateProviderIdArray(insertList);

            co_await DepopulateAsync(deleteList);
            VerifyExist(deleteList, false);
        ...
        // This is where the test begins.
        {
            ...
            // Play inserts that will be undo.
            auto contextArray = co_await this->ApplyMultipleAsync(
                insertList,
                insertIdList,
                0,
            ...
            contextArray = co_await this->ApplyMultipleAsync(
                ...
                0,
                deleteList.Count(),
                checkpointLSN + insertList.Count(),
                checkpointLSN + insertList.Count(),
                ApplyType::Delete,
                ApplyContext::RecoveryRedo,
                secondaryTransactionalReplicator->StateManager.RawPtr());
            ...
            VerifyExist(checkpointList, true, secondaryTransactionalReplicator->StateManager.RawPtr());
            VerifyExist(insertList, false, secondaryTransactionalReplicator->StateManager.RawPtr());
            VerifyExist(deleteList, true, secondaryTransactionalReplicator->StateManager.RawPtr());
    ...
    Awaitable<void> CopyTest::PrepareCheckpointTask(
        __in AwaitableCompletionSource<bool> & signalCompletion) noexcept
    {
    ...
    Awaitable<void> CopyTest::PerformCheckpoint(
        __in AwaitableCompletionSource<bool> & signalCompletion,
        __in bool isCopyFirst,
        __in_opt AwaitableCompletionSource<bool> * const copyCompletion) noexcept
    {
    ...
    Awaitable<void> CopyTest::CompleteCheckpointTask(
        __in AwaitableCompletionSource<bool> & signalCompletion) noexcept
    {
    ...
    Awaitable<void> CopyTest::Test_Copy_PrepareCheckpoint_Race_CopyBasepoint(
        __in ULONG32 numAddsBeforeCheckpoint,
        __in ULONG32 numChangesAfterCheckpoint,
        __in bool isAddChange,
        __in bool recoverInPlace) noexcept
    ...
    Awaitable<void> CopyTest::Test_Copy_PerformCheckpoint_Race(
        __in ULONG32 numAddsBeforeCheckpoint,
        __in ULONG32 numChangesAfterCheckpoint,
        __in bool isAddChange,
        __in bool recoverInPlace,
    ...
    Awaitable<void> CopyTest::Test_Copy_CompleteCheckpoint_Race(
        __in ULONG32 numAddsBeforeCheckpoint,
        __in ULONG32 numChangesAfterCheckpoint,
        __in bool isAddChange,
        __in bool recoverInPlace) noexcept
    ...
    Awaitable<void> CopyTest::Test_Copy_MetadataSize32KOr64K(
        __in bool isFileSize32K,
        __in bool recoverInPlace) noexcept
    {
        NTSTATUS status = STATUS_UNSUCCESSFUL;
    ...
    Awaitable<void> CopyTest::Test_PrepareCheckpoint_CallTwice_CheckpointSecondPrepare(
        __in bool recoverInPlace) noexcept
    {
        NTSTATUS status = STATUS_UNSUCCESSFUL;
    ...
    //    4. Secondary gets the copy stream
    //    5. Verify Secondary copied all the states in the second PrepareCheckpoint
    Awaitable<void> CopyTest::Test_Copy_StateProvider_Empty_State(
        __in ULONG32 numAddsBeforeCheckpoint,
        __in ULONG32 numChangesAfterCheckpoint,
        __in bool recoverInPlace) noexcept
    {
    ...
    BOOST_FIXTURE_TEST_SUITE(CopyTestSuite, CopyTest)
    ...
    BOOST_AUTO_TEST_CASE(Apply_CopyDelete_ApplyDelete_SoftDeleted)
    {
        SyncAwait(this->Test_Apply_CopyDeleted_InsertAndDelete_ItemDeleted(16, false, false));
    }

    BOOST_AUTO_TEST_CASE(Apply_CopyDelete_ApplyInsertDelete_SoftDeleted)
    {
        SyncAwait(this->Test_Apply_CopyDeleted_InsertAndDelete_ItemDeleted(16, true, false));
    }

    BOOST_AUTO_TEST_CASE(Apply_FalseProgress_UndoInsert_UndoDelete_Undone)
    {
        SyncAwait(this->Test_Apply_FalseProgress_Reverted(32));
    }
