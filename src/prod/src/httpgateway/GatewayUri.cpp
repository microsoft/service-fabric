    ...
    __out GatewayUri &output)
{
    ...
    if (!gatewayuri.Parse(validHandlerUris, suffix, uri.Query, verb)) { return false; }

    output = std::move(gatewayuri);
    return true;
}

bool GatewayUri::Parse(vector<HandlerUriTemplate> const& validHandlerUris, wstring const& suffix, wstring const& requestQuery, wstring const& verb)
{
    bool result = false;
    ...
    {
        result = ParseInternal(validHandlerUris[i], verb, requestSuffixVector, requestQuery);
    ...
    wstring const& requestQuery)
{
    ...
    // NOTE: For Backup Restore prefix, do not parse since the suffix format may not be in conformance with the gateway URI schema
    // suffix will be passed to the service as is.
    if (suffixPathBase == Constants::BackupRestorePrefix)
    {
...
bool GatewayUri::UpdateKey(wstring &keyName, wstring &keyValue, wstring const &handlerToken, wstring const &suffixToken)
{
    ...
    if (keyName.empty())
    {
       ...
       if (!Common::StringUtility::StartsWith(temp, *itemDelimiterStart) ||
           !Common::StringUtility::EndsWith(temp, *itemDelimiterEnd))
       {
           return false;
       }
