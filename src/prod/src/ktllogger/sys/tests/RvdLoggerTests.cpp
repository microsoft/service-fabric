...
RvdAsnIndexTests()
{
    {   //** Prove basic AsnIndex behavior and KIoBufferStream access to AsnIndex contents
        ...
        for (ULONGLONG ix = LowestAsn; ix <= HighestAsn; ix++)
        {
            ...
            status = testIndex.AddOrUpdate(indexEntry, resultingIndexEntry, savedState, prevStateStored);
            if (!NT_SUCCESS(status))
            {
    ...
    {   //** Prove AsnIndex order maintainance behaviors - e.g. maintain correct state supporting truncation in light of
        ...
        NTSTATUS                        status = testIndex.AddOrUpdate(indexEntry, resultIndexEntry, savedState, savedStatePresent);
        ...
        status = testIndex.AddOrUpdate(indexEntry, resultIndexEntry, savedState, savedStatePresent);
        KInvariant(NT_SUCCESS(status));
        KInvariant(!savedStatePresent);
