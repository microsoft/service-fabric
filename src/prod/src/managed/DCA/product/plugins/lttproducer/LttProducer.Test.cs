...
Option = m
        ...
        public static void EtlProducerTestSetup(object testContext)
        {
            ...
            Utility.InitializeConfigStore(null);

            Utility.InitializeTracing();
            Utility.TraceSource = FabricEvents.Extensions;
            return;
        }
        ...
        private void LoadManifest(string manifestFile)
        {
            ...
            if (manifestFileName.StartsWith(WinFabricManifestManager.FabricManifestPrefix))
            {
                LoadManifest(manifestFile, ref this.currentFabricManifest);
            }
            else if (manifestFileName.StartsWith(WinFabricManifestManager.LeaseLayerManifestPrefix))
            {
                LoadManifest(manifestFile, ref this.currentLeaseLayerManifest);
            }
            else if (manifestFileName.StartsWith(WinFabricManifestManager.KtlManifestPrefix))
            {
