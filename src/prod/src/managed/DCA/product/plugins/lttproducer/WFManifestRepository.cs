...
namespace FabricDCA
{
    ...
    internal class WFManifestRepository
    {
        ...
        private const string ClusterVersionFileName = "ClusterVersion";
        ...
        internal string RepositoryPath
        {
            get
            {
                lock (this)
                {
                    ...
                    VersionInfo versionInfo = VersionInfo.GetVersionInfoFromFile(
                        Path.Combine(Utility.DcaProgramDirectory, ClusterVersionFileName));

                    // Source location of the manifest files
                    string manifestFileSourceLocation = Utility.DcaProgramDirectory;
        ...
        {
            public readonly string FileMajorPart;
            public readonly string FileMinorPart;
            public readonly string FileBuildPart;
            public readonly string FilePrivatePart;
            ...
            public VersionInfo(string versionString)
            {
                ...
                if (!match.Success)
                {
                    throw new System.ArgumentException($"Unable to extract version information in format Major.Minor.Build.Private from {versionString}", "versionString");
                }

                this.FileMajorPart = match.Groups[1].ToString();
                this.FileMinorPart = match.Groups[2].ToString();
                this.FileBuildPart = match.Groups[3].ToString();
                this.FilePrivatePart = match.Groups[4].ToString();
            }

            public static VersionInfo GetVersionInfoFromFile(string versionFile)
            {
                string version;
