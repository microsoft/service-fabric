...
namespace System.Fabric.BackupRestore.BackupRestoreEndPoints.Operations
{
    ...
    internal class SuspendBackupOperation : BackupMappingOperation<HttpResponseMessage>
    {
        private readonly string _fabricRequestHeader;
        
        public SuspendBackupOperation(string fabricRequestHeader,string apiVersion,  StatefulService statefulService) : base(apiVersion, statefulService)
        {
        ...
        internal override async Task<HttpResponseMessage> RunAsync(TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            switch (fabricBackupResourceType)
            {
                case FabricBackupResourceType.ApplicationUri:
                    workItem = new ResolveToPartitionWorkItem(FabricBackupResourceType.ApplicationUri, applicationNameUri, new WorkItemInfo { WorkItemType = WorkItemPropogationType.SuspendPartition });
                    break;
                case FabricBackupResourceType.ServiceUri:
                    workItem = new ResolveToPartitionWorkItem(FabricBackupResourceType.ServiceUri, serviceNameUri, new WorkItemInfo { WorkItemType = WorkItemPropogationType.SuspendPartition });
                    break;
                case FabricBackupResourceType.PartitionUri:
                    workItem = new SendToServiceNodeWorkItem(serviceNameUri, partitionId, new WorkItemInfo { WorkItemType = WorkItemPropogationType.SuspendPartition });
                    break;
            }
