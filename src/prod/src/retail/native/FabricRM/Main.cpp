...
StringLiteral const TraceComponent("RepairManagerServiceMain");
...
#endif
{
    try
    {
        ...
        auto factorySPtr  = make_shared<RepairManagerServiceFactory>(
            fabricRuntimeCPtr,
            fabricNodeConfig,
        ...
        if(!error.IsSuccess())
        {
            Trace.WriteError(TraceComponent, "wmain(): Failed to create RepairManagerServiceFactory. Error:{0}", error);
