...
namespace MS.Test.ImageStoreService
{
    ...
    internal class ServiceEventListener : EventListener
    {
        ...
        private string filepath = Path.GetDirectoryName(new Uri(Assembly.GetExecutingAssembly().CodeBase).LocalPath);
        private Object fileLock = new Object(); // allows only one writer to file at a time.

        public ServiceEventListener(string appName)
        {
            this.fileName = appName + DateTime.Now.ToString("yyyyMMddHHmmssffff") + ".out";
        }
        ...
        protected override void OnEventWritten(EventWrittenEventArgs eventData)
        {
            lock (this.fileLock)
            {
                using (StreamWriter writer = new StreamWriter(new FileStream(Path.Combine(this.filepath, this.fileName), FileMode.Append)))
                {
