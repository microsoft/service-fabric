...
int main(int, char**)
{
    ...
    {
        ComPointer<TStoreService> service = make_com<TStoreService>(partitionId, replicaId, root);
    ...
    ComPointer<ComFactory> comFactory = make_com<ComFactory>(L"TStoreNightWatchTXRServiceType", *factory.get());
    ...
    fabricRuntime->RegisterStatefulServiceFactory(
        L"TStoreNightWatchTXRServiceType",
